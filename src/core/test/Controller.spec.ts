...
describe("Controller", function(){
    describe("View", function(){
        it("Can pass variables to view", function(){
            ...
            controller.getRouter().createDefaultRoute(".Main");
            let response = TestUtility.render(controller, [
                TestUtility.createTemplate("Called: ${counter}", "Main")
            ]);
        ...
        it("Calls view only once, even if multiple Blocks are present", function(){
            ...
            let response = TestUtility.render(controller, [
                TestUtility.createTemplate("<f:block name='block1' /><f:block name='block2' />", "Main"),
                TestUtility.createTemplate("Called: ${counter}", "template1"),
                TestUtility.createTemplate("Called: ${counter}", "template2")
            ]);
            assert.equal(
                response.getBody(), 
                "<div id=\"f-block-block1\">Called: 1</div><div id=\"f-block-block-2\">Called: 1</div>"
            );
        });
    });
});