...
import { Controller } from "@fortles/core";
...
describe("Application", function () {
    describe("Update mechanism", function () {
        it("Handles menu change with templates", function () {
            ...
            assert.equal(application.simulate(new TestRequest()).getBody(), "<div id=\"f-block-nav\">menu</div><div id=\"f-block-content\">home</div>", "Home page not correct!");
            assert.equal(application.simulate(new TestRequest("/about-us")).getBody(), "<div id=\"f-block-nav\">menu</div><div id=\"f-block-content\">about-us</div>", "About-us page not correct!");
            let request = new TestRequest("/about-us", 1 /* RequestType.PARTIAL */);
            request.referer = "/";
            request.blockPath = "menu";
            let response = application.simulate(request);
            assert.equal(response.getBody(), "about-us", "Partial request not correct!");
            assert.equal(response.blockPath, "content", "Target is not properly set!");
        });
